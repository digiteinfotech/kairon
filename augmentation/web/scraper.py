...
class WebScraper:
    ...
    def join_bulletins(text: list):
        ...
        for i in range(1, len(text)):
            if text[i][
                0] == '-':  # bulletins have '-' as the first character in the string. They are appended in the last added string in list better_text
    ...
    @staticmethod
    def remove_headers_and_separate_headers_joined_with_regular_text(text: list, soup):
        """
        ...
        for i in text:
            if i in all_header_text:
            ...
            elif any([j for j in all_header_text if
                      j in i]):  # if any of the header is in 'i' then this condition is satisfied
                ...
                for j in all_header_text:
                    if j in i:
                        tmp_ls = i.split(j)
                        if len(tmp_ls[0]) == 0 and len(
                                tmp_ls) != 0:  # if the first element of tmp_ls is of len=0 then a header was present in the beginning
                            good = True
                            candidate = candidate if len(candidate) > len(
                                j) else j  # to store the biggest matching header in candidate
    ...
    def get_text(url: str, pages: list, max_pages: int):
        ...
        try:
            ...
            text = trafilatura.extract(request.text, include_comments=False, include_tables=False, include_links=False,
                                       target_language='en')  # extract relevant text from current page
            text = text.replace("READ MORE", "")
            text = text.replace("LEARN MORE", "")
            text = text.split("\n")
            text = WebScraper.join_bulletins(text)
            text = WebScraper.remove_headers_and_separate_headers_joined_with_regular_text(text, soup)
