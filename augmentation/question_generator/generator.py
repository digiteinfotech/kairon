...
class QuestionGenerator:
    ...
    generator_args = {
        'temperature': os.getenv('QUESTION_GENERATOR_TEMPERATURE', 1.02),
        'num_beams': os.getenv('QUESTION_GENERATOR_NUM_BEANS', 1),
        'max_length': os.getenv('QUESTION_GENERATOR_MAX_LENGTH', 70)
    }
    ...
    def generate(text: str):
        ...
        res = QuestionGenerator.model.generate(input_ids, **QuestionGenerator.generator_args)
        output = QuestionGenerator.tokenizer.batch_decode(res, skip_special_tokens=True)
        output = output[0].split(QuestionGenerator.tokenizer.sep_token)
        if output and (len(output[-1]) == 0 or output[-1][-1] != "?"):
            output.pop()
