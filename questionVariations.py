...
m = gensim.models.KeyedVectors.load_word2vec_format(r'./wiki-news-300d-1M.vec') #enter the load path of model
...
class Variate():
    ...
    def preprocess_text(self,text,custom_stopwords,word_len,white_space=True,lower_case=True,number=True):
    ...
    def comb(self,lis):
        ...
        for sent in lis:
            testr = self.preprocess_text(sent,entity_helpverb_words,0,white_space=True,lower_case=True,number=True)
            testf = self.preprocess_text(sent,[],0,white_space=True,lower_case=True,number=True)
