...
class TestBusinessServiceProvider:
    ...
    def test_get_auth_token(self, monkeypatch):
        ...
        monkeypatch.setitem(Utility.environment["channels"]["360dialog_on_premise"], "partner_username",
                            partner_username)
        monkeypatch.setitem(Utility.environment["channels"]["360dialog_on_premise"], "partner_password",
                            partner_password)
    ...
    @responses.activate
    def test_get_auth_token_bsp_cloud(self, monkeypatch):
        partner_username = "udit"
        partner_password = "Test@test"
        monkeypatch.setitem(Utility.environment["channels"]["360dialog_cloud"], "partner_username",
                            partner_username)
        monkeypatch.setitem(Utility.environment["channels"]["360dialog_cloud"], "partner_password",
                            partner_password)
        base_url = Utility.system_metadata["channels"]["whatsapp"]["business_providers"]["360dialog_cloud"][
            "hub_base_url"]
        url = f"{base_url}/api/v2/token"
        api_resp = {
            "token_type": "bearer", "access_token": "sdfghjkl;34567890-"
        ...
        responses.add("POST", json=api_resp, url=url,
                      match=[
                          responses.matchers.json_params_matcher(
                              {"username": partner_username, "password": partner_password})])
        actual = BSP360DialogCloud.get_partner_auth_token()
        assert actual == api_resp.get("token_type") + " " + api_resp.get("access_token")
    ...
    @responses.activate
    def test_get_auth_token_error_bsp_cloud(self):
        base_url = Utility.system_metadata["channels"]["whatsapp"]["business_providers"]["360dialog_cloud"][
            "hub_base_url"]
        url = f"{base_url}/api/v2/token"
        responses.add("POST", json={}, url=url, status=500)
        with pytest.raises(AppException, match=r"Failed to get partner auth token: *"):
            BSP360DialogCloud.get_partner_auth_token()
    ...
    def test_get_account(self, monkeypatch):
        ...
        monkeypatch.setitem(Utility.environment["channels"]["360dialog_on_premise"], "partner_id", partner_id)
        base_url = Utility.system_metadata["channels"]["whatsapp"]["business_providers"]["360dialog_on_premise"][
            "hub_base_url"]
    ...
    @responses.activate
    def test_get_account_bsp_cloud(self, monkeypatch):
        channel_id = "skds23Ga"
        partner_id = "jhgajfdk"
        ...
        monkeypatch.setattr(BSP360DialogCloud, 'get_partner_auth_token', _get_partners_auth_token)
        monkeypatch.setitem(Utility.environment["channels"]["360dialog_cloud"], "partner_id", partner_id)
        base_url = Utility.system_metadata["channels"]["whatsapp"]["business_providers"]["360dialog_cloud"][
            "hub_base_url"]
        url = f"{base_url}/api/v2/partners/{partner_id}/channels?filters={{'id':'{channel_id}'}}"
        api_resp = {
    ...
    @responses.activate
    def test_get_account_failure_bsp_cloud(self, monkeypatch):
        channel_id = "skds23Ga"
        partner_id = "jhgajfdk"
    ...
    @responses.activate
    def test_get_account_auth_failure_bsp_cloud(self, monkeypatch):
        channel_id = "skds23Ga"
        base_url = Utility.system_metadata["channels"]["whatsapp"]["business_providers"]["360dialog_cloud"][
            "hub_base_url"]
        url = f"{base_url}/api/v2/token"
    ...
    @responses.activate
    def test_set_webhook_url_bsp_cloud(self, monkeypatch):
        def _get_partners_auth_token(*args, **kwargs):
            return "Bearer eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCIs.ImtpZCI6Ik1EZEZOVFk1UVVVMU9FSXhPRGN3UVVZME9EUTFRVFJDT1.RSRU9VUTVNVGhDTURWRk9UUTNPQSJ9"
        ...
        webhook_url = BSP360DialogCloud.set_webhook_url(api_key, webhook_url)
        assert webhook_url == "https://kaironlocalchat.digite.com/api/bot/waba_partner/62bc24b493a0d6b7a46328f5/eyJhbGciOiJIUzI1NiI.sInR5cCI6IkpXVCJ9.TXXmZ4-rMKQZMLwS104JsvsR0XPg4xBt2UcT4x4HgLY"
    ...
    @responses.activate
    def test_set_webhook_url_failure_bsp_cloud(self):
        url = "https://waba.360dialog.io/v1/configs/webhook"
        responses.add("POST", json={}, url=url, status=500)
        webhook_url = "https://kaironlocalchat.digite.com/api/bot/waba_partner/62bc24b493a0d6b7a46328f5/eyJhbGciOiJIUzI1NiI.sInR5cCI6IkpXVCJ9.TXXmZ4-rMKQZMLwS104JsvsR0XPg4xBt2UcT4x4HgLY"
        api_key = "kHCwksdsdsMVYVx0doabaDyRLUQJUAK"
    ...
    def test_generate_waba_key(self, monkeypatch):
        ...
        monkeypatch.setitem(Utility.environment["channels"]["360dialog_on_premise"], "partner_username", 'testuser')
        monkeypatch.setitem(Utility.environment["channels"]["360dialog_on_premise"], "partner_password", 'testpassword')
    ...
    @responses.activate
    def test_generate_waba_key_bsp_cloud(self, monkeypatch):
        def _get_partners_auth_token(*args, **kwargs):
            return "Bearer eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCIs.ImtpZCI6Ik1EZEZOVFk1UVVVMU9FSXhPRGN3UVVZME9EUTFRVFJDT1.RSRU9VUTVNVGhDTURWRk9UUTNPQSJ9"

        monkeypatch.setattr(BSP360DialogCloud, 'get_partner_auth_token', _get_partners_auth_token)
        monkeypatch.setitem(Utility.environment["channels"]["360dialog_cloud"], "partner_id", 'f167CmPA')
        monkeypatch.setitem(Utility.environment["channels"]["360dialog_cloud"], "partner_username", 'testuser')
        monkeypatch.setitem(Utility.environment["channels"]["360dialog_cloud"], "partner_password", 'testpassword')
        url = "https://hub.360dialog.io/api/v2/partners/f167CmPA/channels/skds23Ga/api_keys"
        responses.add("POST",
                      json={
                          "address": "https://waba.360dialog.io",
        ...
        api_key = BSP360DialogCloud.generate_waba_key("skds23Ga")
        assert api_key == "kHCwksdsdsMVYVx0doabaDyRLUQJUAK"
    ...
    @responses.activate
    def test_generate_waba_key_failure_bsp_cloud(self, monkeypatch):
        url = "https://hub.360dialog.io/api/v2/token"
        responses.add("POST", json={}, url=url, status=500)
        with pytest.raises(AppException, match=r"Failed to get partner auth token: *"):
            BSP360DialogCloud.generate_waba_key("skds23Ga")
    ...
    def test_save_channel_config_bsp_disabled(self, monkeypatch):
        ...
        monkeypatch.setitem(Utility.environment["channels"]["360dialog_on_premise"], 'partner_id', "test_id")
    ...
    def test_save_channel_config_bsp_disabled_bsp_cloud(self, monkeypatch):
        bot = "62bc24b493a0d6b7a46328f5"
        user = "test_user"
        client_name = "kairon"
        client_id = "skds23Ga"
        ...
        monkeypatch.setitem(Utility.environment['model']['agent'], 'url', "http://kairon-api.digite.com")
        monkeypatch.setitem(Utility.environment["channels"]["360dialog_cloud"], 'partner_id', "test_id")
        monkeypatch.setattr(Authentication, 'generate_integration_token', _get_integration_token)
    ...
    def test_save_channel_config_bsp_cloud(self, monkeypatch):
        bot = "62bc24b493a0d6b7a46328f5"
        user = "test_user"
        client_name = "kairon"
        client_id = "skds23Ga"
        ...
        def _mock_get_bot_settings(*args, **kwargs):
            return BotSettings(whatsapp="360dialog_cloud", bot=bot, user=user)
        ...
        config = ChatDataProcessor.get_channel_config("whatsapp", bot, mask_characters=False)
        assert config['config'] == {'client_name': 'kairon', 'client_id': 'skds23Ga', 'channel_id': 'dfghjkl',
                                    'partner_id': 'test_id', 'bsp_type': '360dialog_cloud',
                                    'api_key': 'kHCwksdsdsMVYVx0doabaDyRLUQJUAK', 'waba_account_id': 'Cyih7GWA'}
    ...
    def test_save_channel_config_with_partner_id_bsp_cloud(self, monkeypatch):
        bot = "62bc24b493a0d6b7a46328ff"
        user = "test_user"
        client_name = "kairon"
        client_id = "skds23Ga"
        ...
        def _mock_get_bot_settings(*args, **kwargs):
            return BotSettings(whatsapp="360dialog_cloud")
        ...
        config = ChatDataProcessor.get_channel_config("whatsapp", bot, mask_characters=False)
        assert config['config'] == {'client_name': 'kairon', 'client_id': 'skds23Ga', 'channel_id': 'dfghjkl',
                                    'partner_id': partner_id, 'bsp_type': '360dialog_cloud',
                                    'api_key': 'kHCwksdsdsMVYVx0doabaDyRLUQJUAK', 'waba_account_id': 'Cyih7GWA'}
    ...
    @responses.activate
    def test_get_template_bsp_cloud(self, monkeypatch):
        bot = "62bc24b493a0d6b7a46328ff"
        template_id = "test_id"
        partner_id = "new_partner_id"
        account_id = "Cyih7GWA"
    ...
    @responses.activate
    def test_get_template_failure_bsp_cloud(self, monkeypatch):
        bot = "62bc24b493a0d6b7a46328ff"
        template_id = "test_id"
        partner_id = "new_partner_id"
        account_id = "Cyih7GWA"
    ...
    def test_get_template_error_bsp_cloud(self, monkeypatch):
        bot = "62bc24b493a0d6b7a46328fs"
        template_id = "test_id"
    ...
    def test_post_process_bsp_cloud(self, monkeypatch):
        def _generate_waba_key(*args, **kwargs):
            return "kHCwksdsdsMVYVx0doabaDyRLUQJUAK"
        ...
        monkeypatch.setattr(BSP360DialogCloud, 'generate_waba_key', _generate_waba_key)
        monkeypatch.setattr(BSP360DialogCloud, 'get_account', _get_waba_account_id)
        monkeypatch.setattr(BSP360DialogCloud, 'set_webhook_url', _set_webhook_url)
        monkeypatch.setattr(Authentication, 'generate_integration_token', _get_integration_token)
        monkeypatch.setitem(Utility.environment['model']['agent'], 'url', "http://kairon-api.digite.com")
        ...
        url = ChatDataProcessor.save_channel_config(config, "62bc24b493a0d6b7a46328f3", "test@demo.in")
        assert url == 'http://kairon-api.digite.com/api/bot/whatsapp/62bc24b493a0d6b7a46328f3/eyJhbGciOiJIUzI1NiI.sInR5cCI6IkpXVCJ9.TXXmZ4-rMKQZMLwS104JsvsR0XPg4xBt2UcT4x4HgLY'

        webhook_url = BSP360DialogCloud("62bc24b493a0d6b7a46328f3", "test@demo.in").post_process()
        assert webhook_url == 'http://kairon-api.digite.com/api/bot/whatsapp/62bc24b493a0d6b7a46328f3/eyJhbGciOiJIUzI1NiI.sInR5cCI6IkpXVCJ9.TXXmZ4-rMKQZMLwS104JsvsR0XPg4xBt2UcT4x4HgLY'
    ...
    def test_post_process_bsp_disabled_bsp_cloud(self, monkeypatch):
        def _generate_waba_key(*args, **kwargs):
            return "kHCwksdsdsMVYVx0doabaDyRLUQJUAK"
        ...
        def _set_webhook_url(*args, **kwargs):
            return "https://kaironlocalchat.digite.com/api/bot/waba_partner/62bc24b493a0d6b7a46328f3/eyJhbGciOiJIUzI1NiI.sInR5cCI6IkpXVCJ9.TXXmZ4-rMKQZMLwS104JsvsR0XPg4xBt2UcT4x4HgLY"
        ...
        with pytest.raises(AppException, match="Feature disabled for this account. Please contact support!"):
            BSP360DialogCloud("62bc24b493a0d6b7a46328f3", "test@demo.in").post_process()
    ...
    @responses.activate
    def test_post_process_error_bsp_cloud(self):
        base_url = Utility.system_metadata["channels"]["whatsapp"]["business_providers"]["360dialog_cloud"][
            "hub_base_url"]
        url = f"{base_url}/api/v2/token"
        responses.add("POST", json={}, url=url, status=500)
    ...
    def test_post_process_client_config_deleted_bsp_cloud(self):
        with pytest.raises(AppException, match="Channel not found!"):
            BSP360DialogCloud("test_bot", "test@demo.in").post_process()
    ...
    def test_bsp_factory_with_bsp_cloud(self):
        assert isinstance(
            BusinessServiceProviderFactory.get_instance(WhatsappBSPTypes.bsp_360dialog_cloud.value)("test", "test"),
            BSP360DialogCloud)
