...
class TestCustomerUpsert:
    ...
    def test_upsert_customer_create_success(self):
        ...
        result = CustomerOrderProcessor.upsert_customer(
            bot=bot, user="tester", encrypted_id=enc,
            persona_type="fnb",
    ...
    def test_upsert_customer_update_existing(self):
        ...
        assert CustomerDetails.objects(bot=bot, user_id="user_u1").count() == 1
    ...
    def test_upsert_customer_invalid_persona_type(self):
        bot = "cop_upsert_bad_persona_bot"
        enc = _enc("user_bp1")
        with pytest.raises(AppException):
            CustomerOrderProcessor.upsert_customer(
                bot=bot, user="tester", encrypted_id=enc,
                persona_type="unknown_type",
                payload={"mobile": "9000000004"},
            )

    def test_upsert_customer_missing_required_field_fnb(self):
        bot = "cop_upsert_missing_bot"
        enc = _enc("user_m1")
        with pytest.raises(AppException):
            CustomerOrderProcessor.upsert_customer(
                bot=bot, user="tester", encrypted_id=enc,
    ...
    def test_upsert_customer_invalid_encrypted_id(self):
        with pytest.raises(AppException, match="Invalid identifier"):
            CustomerOrderProcessor.upsert_customer(
                bot="any_bot", user="tester", encrypted_id="not_valid_encrypted",
                persona_type="fnb",
                payload={"mobile": "9000000005"},
            )

    def test_upsert_customer_response_re_encrypts_user_id(self):
        bot = "cop_mask_bot"
        plain = "user_mask1"
        enc = _enc(plain)
        result = CustomerOrderProcessor.upsert_customer(
        ...
        assert result["user_id"] != plain
        assert Utility.decrypt_message(result["user_id"]) == plain
...
class TestCustomerGet:
    ...
    def setup_method(self):
        ...
        CustomerOrderProcessor.upsert_customer(
            bot=self.bot, user="tester", encrypted_id=self.enc,
            persona_type="fnb",
            payload={"name": "GetUser", "mobile": "9100000001"},
        )

    def test_get_customer_success(self):
        result = CustomerOrderProcessor.get_customer(bot=self.bot, encrypted_id=self.enc)
        assert result["name"] == "GetUser"
        assert result["mobile"] == "9100000001"

    def test_get_customer_masks_user_id(self):
        result = CustomerOrderProcessor.get_customer(bot=self.bot, encrypted_id=self.enc)
        assert result["user_id"] != self.plain_id
        assert Utility.decrypt_message(result["user_id"]) == self.plain_id

    def test_get_customer_not_found(self):
        enc = _enc("user_nonexistent")
        with pytest.raises(AppException, match="Customer not found"):
            CustomerOrderProcessor.get_customer(bot=self.bot, encrypted_id=enc)

    def test_get_customer_wrong_bot(self):
        with pytest.raises(AppException, match="Customer not found"):
            CustomerOrderProcessor.get_customer(bot="wrong_bot", encrypted_id=self.enc)
    ...
    def test_get_customer_deleted_not_found(self):
        ...
        CustomerOrderProcessor.delete_customer(bot=bot, encrypted_id=enc)
        with pytest.raises(AppException, match="Customer not found"):
            CustomerOrderProcessor.get_customer(bot=bot, encrypted_id=enc)
...
class TestCustomerList:
    ...
    def test_list_customers_all(self):
        ...
        for i in range(3):
            CustomerOrderProcessor.upsert_customer(
                bot=bot, user="tester", encrypted_id=_enc(f"list_user_{i}"),
    ...
    def test_list_customers_filter_by_persona_type(self):
        ...
        CustomerOrderProcessor.upsert_customer(
            bot=bot, user="tester", encrypted_id=_enc("lp_user1"),
            persona_type="fnb", payload={"mobile": "9200000001"},
        )
        CustomerOrderProcessor.upsert_customer(
            bot=bot, user="tester", encrypted_id=_enc("lp_user2"),
    ...
    def test_list_customers_pagination(self):
        ...
        for i in range(5):
            CustomerOrderProcessor.upsert_customer(
                bot=bot, user="tester", encrypted_id=_enc(f"page_user_{i}"),
    ...
    def test_list_customers_excludes_deleted(self):
        ...
        CustomerOrderProcessor.upsert_customer(
            bot=bot, user="tester", encrypted_id=enc_keep,
            persona_type="fnb", payload={"mobile": "9400000001"},
        )
        CustomerOrderProcessor.upsert_customer(
            bot=bot, user="tester", encrypted_id=enc_del,
            persona_type="fnb", payload={"mobile": "9400000002"},
        )
        CustomerOrderProcessor.delete_customer(bot=bot, encrypted_id=enc_del)
        result = CustomerOrderProcessor.list_customers(bot=bot)
        assert len(result) == 1
...
class TestUpdateAddress:
    ...
    def test_update_address_add_new_label(self):
        result = CustomerOrderProcessor.update_address(
            bot=self.bot, encrypted_id=self.enc,
    ...
    def test_update_address_customer_not_found(self):
        with pytest.raises(AppException, match="Customer not found"):
            CustomerOrderProcessor.update_address(
                bot=self.bot, encrypted_id=_enc("no_such_user"),
...
class TestDeleteCustomer:
    ...
    def test_delete_customer_success(self):
        ...
        CustomerOrderProcessor.delete_customer(bot=bot, encrypted_id=enc)
        assert CustomerDetails.objects(bot=bot, user_id="del_user1", status=False).count() == 1
        assert CustomerDetails.objects(bot=bot, user_id="del_user1", status=True).count() == 0

    def test_delete_customer_not_found(self):
        with pytest.raises(AppException, match="Customer not found"):
            CustomerOrderProcessor.delete_customer(
                bot="cop_del_bot", encrypted_id=_enc("no_such_del_user"),
            )
    ...
    def test_delete_customer_is_soft_delete(self):
        ...
        CustomerOrderProcessor.delete_customer(bot=bot, encrypted_id=enc)
        assert CustomerDetails.objects(bot=bot, user_id="soft_del_user1").count() == 1
    ...
    def test_create_order_customer_not_found(self):
        with pytest.raises(AppException, match="Customer not found"):
            CustomerOrderProcessor.create_order(
                bot=self.bot, user="tester", encrypted_id=_enc("no_customer"),
                persona_type="fnb",
                order_payload={"item": "Soda"},
            )

    def test_create_order_masks_user_id_in_response(self):
        result = CustomerOrderProcessor.create_order(
            bot=self.bot, user="tester", encrypted_id=self.enc,
            persona_type="fnb",
            order_payload={"item": "Tea", "qty": 3, "price": 50},
        )
        assert result["user_id"] != "order_user1"
        assert Utility.decrypt_message(result["user_id"]) == "order_user1"
...
class TestGetOrder:
    ...
    def setup_method(self):
        ...
        CustomerOrderProcessor.upsert_customer(
            bot=self.bot, user="tester", encrypted_id=enc,
...
class TestListOrders:
    ...
    def setup_class(cls):
        ...
        CustomerOrderProcessor.upsert_customer(
            bot=cls.bot, user="tester", encrypted_id=cls.enc,
    ...
    def test_list_orders_for_customer_pagination(self):
        page1 = CustomerOrderProcessor.list_orders_for_customer(
            bot=self.bot, encrypted_id=self.enc, page=1, page_size=2,
        )
        page2 = CustomerOrderProcessor.list_orders_for_customer(
            bot=self.bot, encrypted_id=self.enc, page=2, page_size=2,
        )
        assert len(page1) == 2
        assert len(page2) == 2
    ...
    def test_list_orders_for_customer_empty(self):
        result = CustomerOrderProcessor.list_orders_for_customer(
            bot=self.bot, encrypted_id=_enc("nonexistent_order_user"),
        )
        assert result == []
...
class TestFilterOrders:
    ...
    def setup_class(cls):
        ...
        CustomerOrderProcessor.upsert_customer(
            bot=cls.bot, user="tester", encrypted_id=enc,
