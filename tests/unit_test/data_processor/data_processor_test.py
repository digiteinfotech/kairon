...
class TestModelProcessor:
    ...
    def test_auditlog_for_chat_client_config(self):
        ...
        assert auditlog_data[0].metadata["value"] == "test"
        assert auditlog_data[0].user == "testUser"
        assert auditlog_data[0].entity == "ChatClientConfig"

    def test_auditlog_for_intent(self):
        auditlog_data = list(
            AuditLogData.objects(metadata={"key": "Bot_id", "value": "tests"}, user='testUser', action='save', entity='Intents').order_by('-timestamp'))
        assert len(auditlog_data) > 0
        assert auditlog_data is not None
        assert auditlog_data[0].metadata["value"] == "tests"
        assert auditlog_data[0].user == "testUser"
    ...
    def test_auditlog_for_data_with_encrypted_field(self):
        ...
        print(auditlog_data)
        assert auditlog_data[2]["metadata"]["value"] == bot
        assert auditlog_data[2]["entity"] == "Channels"
        assert auditlog_data[2]["data"]["config"] != config

        assert auditlog_data[0]["metadata"]["value"] == bot
