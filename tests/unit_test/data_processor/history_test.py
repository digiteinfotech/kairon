...
class TestHistory:
    ...
    def tracker_keys(self, *args, **kwargs):
        return ["5b029887-bed2-4bbb-aa25-bd12fda26244",
    ...
    def user_history(self, *args, **kwargs):
        ...
        return (DialogueStateTracker
                .from_dict("5e564fbcdcf0d5fad89e3acd",json_data['events'], domain.slots)
                .as_dialogue()
                .events)
    ...
    def mock_chat_history(self, monkeypatch):
        ...
        monkeypatch.setattr(ChatHistory, "get_conversations", self.history_conversations)
    ...
    def test_fetch_chat_users_db_error(self, mock_get_tracker_and_domain):
        with pytest.raises(Exception):
            users = ChatHistory.fetch_chat_users(bot="tests")
            assert len(users) == 0

    def test_fetch_chat_users_empty(self, mock_get_tracker_and_domain):
        users = ChatHistory.fetch_chat_users(bot="tests")
        assert len(users) == 0

    def test_fetch_chat_users(self, mock_tracker, mock_get_tracker_and_domain):
        users = ChatHistory.fetch_chat_users(bot="tests")
        assert len(users) == 7

    def test_fetch_chat_history_empty(self, mock_get_tracker_and_domain):
        history = ChatHistory.fetch_chat_history(sender="123",bot="tests")
        assert len(history) == 0

    def test_fetch_chat_history(self, mock_chat_history, mock_get_tracker_and_domain):
        history = ChatHistory.fetch_chat_history(sender="5e564fbcdcf0d5fad89e3acd", bot="tests")
        assert len(history) == 12
        assert history[0]['event']
        assert history[0]['time']
        assert history[0]['date']
        assert history[0]['text']
        assert history[0]['is_exists']
        assert history[0]['intent']
        assert history[0]['confidence']

    def test_visitor_hit_fallback_empty(self, mock_get_tracker_and_domain):
    ...
    def test_visitor_hit_fallback(self, mock_chat_history, mock_get_tracker_and_domain):
        hit_fall_back = ChatHistory.visitor_hit_fallback("tests")
        assert hit_fall_back['fallback_count'] == 3
        assert hit_fall_back['total_count'] == 31

    def test_conversation_time_empty(self, mock_get_tracker_and_domain):
        conversation_time = ChatHistory.conversation_time("tests")
        assert not conversation_time

    def test_conversation_time(self, mock_chat_history, mock_get_tracker_and_domain):
        conversation_time = ChatHistory.conversation_time("tests")
        assert conversation_time

    def test_conversation_steps_empty(self, mock_get_tracker_and_domain):
        conversation_steps = ChatHistory.conversation_steps("tests")
        assert not conversation_steps

    def test_conversation_steps(self, mock_chat_history, mock_get_tracker_and_domain):
        conversation_steps = ChatHistory.conversation_steps("tests")
