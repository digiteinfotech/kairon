...
class TestBotModels:
    ...
    def test_slot(self):
        with pytest.raises(ValueError, match="Slot name and type cannot be empty or blank spaces"):
            Slots(
                name='email_id', type=' ', auto_fill=True, mapping=[{"type": "from_value"}]
            ).save()
        with pytest.raises(ValueError, match="Slot name and type cannot be empty or blank spaces"):
            Slots(
                name=' ', type='text', auto_fill=True, mapping=[{"type": "from_value"}]
            ).save()
    ...
    def test_validate_slot_mapping(self):
        with pytest.raises(ValidationError,
            ...
            Slots(
                name='email_id', type='text', auto_fill=True, mapping=[{"type": "from_value"}]
            ).save()
        assert not Slots(
            name='email_id', type='text', auto_fill=True,
            mapping=[{"type": "from_intent", "value": 'uditpandey@hotmail.com'}]
        ).validate()
