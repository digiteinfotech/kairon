...
def test_add_story_invalid_event_type():
    ...
    assert (
            ...
            == [{'ctx': {'enum_values': ['INTENT', 'BOT', 'HTTP_ACTION', 'ACTION', 'SLOT_SET_ACTION', 'FORM_ACTION']},
                 ...
                 'msg': "value is not a valid enumeration member; permitted: 'INTENT', 'BOT', 'HTTP_ACTION', 'ACTION', 'SLOT_SET_ACTION', 'FORM_ACTION'",
                 'type': 'type_error.enum'}]
    )
...
def test_trigger_mail_on_new_signup_with_sso(monkeypatch):
    ...
    async def __mock_verify_and_process(*args, **kwargs):
        return True, {'email': 'new_user@digite.com', 'first_name': 'new', 'password': SecretStr('123456789')}, token
