...
class MongoProcessor:
    ...
    def save_component_properties(self, configs: dict, bot: Text, user: Text):
        ...
        if nlu_confidence_threshold:
            ...
            fallback = next((comp for comp in present_config['pipeline'] if comp["name"] == "FallbackClassifier"), {})
            if not fallback:
                fallback['name'] = 'FallbackClassifier'
                present_config['pipeline'].append(fallback)
            fallback['threshold'] = nlu_confidence_threshold
            rule_policy = next((comp for comp in present_config['policies'] if comp["name"] == "RulePolicy"), {})
            if not rule_policy:
                rule_policy['name'] = 'RulePolicy'
                present_config['pipeline'].append(rule_policy)
    ...
    def delete_fallback_properties(self, bot: Text, user: Text, delete_nlu_fallback: bool = True, delete_action_fallback: bool = True):
        ...
        if delete_action_fallback:
            index = next((index for index, comp in enumerate(config['policies']) if comp["name"] == "RulePolicy"), None)
            if index:
                del config['policies'][index]
                config['policies'].append({'name': 'RulePolicy'})
