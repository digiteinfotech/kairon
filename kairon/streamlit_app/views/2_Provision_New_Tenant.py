...
render_header("Provision New Tenant", "Strategy-based automated deployment for Standalone Apps (Tier 1) and Monolithic ERP Suites (Tier 2).", icon="🚀")
...
if existing_tenant:
    ...
    if curr_status != "COMPLETED" and "FAILED" not in curr_status:
        st.info("Provisioning is currently recorded as in progress. Please check the 'Tenants Directory' or 'System Logs' for updates.")
        
    st.warning("If you want to start a new deployment for this bot, you must delete the current tenant configuration first from the **Tenants Directory**.")
    st.stop()
...
if submit_btn and selected_features:
    ...
    try:
        ...
        # Call Backend Strategy Provisioner
