...
from kairon.shared.actions.data_objects import ActionServerLogs, PublicSearchAction
...
class ActionPublicSearch(ActionsBase):
    ...
    def retrieve_config(self):
        ...
        :return: PublicSearchAction containing configuration for the action as a dict.
        """
        try:
            action = PublicSearchAction.objects(bot=self.bot, name=self.name, status=True).get().to_mongo().to_dict()
    ...
    async def execute(self, dispatcher: CollectingDispatcher, tracker: Tracker, domain: Dict[Text, Any]):
        ...
        try:
            ...
            if not ActionUtility.is_empty(latest_msg):
                results = ActionUtility.perform_public_search(latest_msg,
            ...
            ActionServerLogs(
                type=ActionType.public_search_action.value,
