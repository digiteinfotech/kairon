...
class ActionProcessor:
    ...
    async def __process_slot_set_action(tracker: Tracker, action_config: dict):
        ...
        elif action_config['type'] == SLOT_SET_TYPE.FROM_SLOT.value:
            message.append(f"Setting slot '{action_config['slot']}' to slot '{action_config['value']}'.")
            value = tracker.get_slot(action_config['value'])
            if action_config['value'] not in tracker.current_slot_values():
                message.append(f"Tried to access non existent slot '{action_config['value']}'. Setting value to None.")
                status = 'FAILURE'
        else:
