...
class IntegrationsProcessor:
    ...
    @staticmethod
    def get_integrations(bot: Text, raise_exception: bool = False):
        """
        ...
        """
        try:
            for i in list(Integrations.objects):
            ...
            logging.error(e)
            if raise_exception:
                raise AppException("No integrations exist")
    ...
    def add_integration(name: Text, issued_at: datetime, bot: Text, user: Text):
        ...
        """
        try:
            if Integrations.objects(bot=bot, status__ne="deleted").count() >= Utility.environment['security']['token_limit']:
