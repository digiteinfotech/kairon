...
class IDPHelper:
    ...
    def get_admin_access_token():
        ...
        body = AUTH_ACCESS_TOKEN_PAYLOAD.format(
    ...
    def upsert_identity_provider(realm_name, operation, **kwargs):
        ...
        if operation == "update":
            ...
            url = Utility.environment["idp"][
                "server_url"
            ] + IDPURLConstants.UPDATE_IDP_TO_REALM_URL.value.format(
        ...
        body = json.dumps(
            IDPPayload.get_idp_config(idp_provider=kwargs.get("config_type"))
        )
        ...
        try:
            ...
            for client in clients:
                if client["clientId"] == client_name:
                    ...
                    idp_client_secret = IDPHelper.get_secret_for_client(
                        realm_name=realm_name, client_id=realm_client_id
                    )
                    if get_service_user:
                        service_account_id = (
                            IDPHelper.get_service_account_user_for_client(
                                realm_name, realm_client_id
                            )
                        )
    ...
    def get_fastapi_idp_object(realm_name):
        ...
        try:
            return FastAPIKeycloak(
                ...
                realm=realm_name,
                callback_uri=Utility.environment["idp"]["callback_frontend_url"]
                + f"/login/idp/callback/{realm_name}",
            )
    ...
    def create_autherization_flow(realm_name):
        url = Utility.environment["idp"][
            "server_url"
        ] + IDPURLConstants.BROWSER_AUTH_FLOW_EXECUTION_URL.value.format(
        ...
        try:
            browser_auth_flows = Utility.execute_http_request(
                "GET", http_url=url, headers=headers
            )
            for auth_flow in browser_auth_flows:
                if auth_flow["providerId"] == "identity-provider-redirector":
                    ...
                    url = Utility.environment["idp"][
                        "server_url"
                    ] + IDPURLConstants.BROWSER_AUTH_FLOW_EXECUTION_CONFIG_URL.value.format(
    ...
    def create_ipd_client(realm_name):
        ...
        try:
            ...
            body = body.replace(
                "REDIRECT_URI",
                Utility.environment["idp"]["callback_frontend_url"]
                + f"/login/idp/callback/{realm_name}",
            )
    ...
    def get_secret_for_client(realm_name, client_id):
        url = Utility.environment["idp"][
            "server_url"
        ] + IDPURLConstants.GET_SECRET_FOR_IDP_CLIENT_URL.value.format(
            realm_name=realm_name, realm_client_id=client_id
        )
        headers = {
            "Authorization": "Bearer " + IDPHelper.get_admin_access_token(),
        }
    ...
    def get_service_account_user_for_client(realm_name, client_id):
        url = Utility.environment["idp"][
            "server_url"
        ] + IDPURLConstants.SERVICE_ACCOUNT_USER_URL.value.format(
            realm_name=realm_name, realm_client_id=client_id
        )
        headers = {
            "Authorization": "Bearer " + IDPHelper.get_admin_access_token(),
    ...
    @staticmethod
    def get_available_role_for_service_account_in_client(
        realm_name, client_id, user_id
    ):
        url = Utility.environment["idp"][
            "server_url"
        ] + IDPURLConstants.AVAILABLE_ROLE_FOR_CLIENT.value.format(
            realm_name=realm_name, realm_client_id=client_id, service_user_id=user_id
        )
        headers = {
            "Authorization": "Bearer " + IDPHelper.get_admin_access_token(),
        }
        try:
            return Utility.execute_http_request(
                "GET", http_url=url, headers=headers, return_json=False
            )
        except AppException:
    ...
    def add_role_to_service_account_in_client(realm_name, user_id):
        ...
        try:
            clients = IDPHelper.get_realm_clients(realm_name)
            for client in clients:
                if client["clientId"] in (
                    IDPClientNames.ACCOUNT.value,
                    IDPClientNames.REALM_MANAGEMENT.value,
                ):
                    resp = IDPHelper.get_available_role_for_service_account_in_client(
                        realm_name, client["id"], user_id
                    )
                    headers = {
                        "Authorization": "Bearer " + IDPHelper.get_admin_access_token()
                    }
    ...
    def delete_realm(realm_name):
        ...
        try:
            return Utility.execute_http_request(
                "DELETE", http_url=url, headers=headers, return_json=False
            )
        except AppException:
