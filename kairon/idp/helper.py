...
class IDPHelper:
    ...
    def get_admin_access_token():
        ...
        try:
            result = Utility.execute_http_request("POST", http_url=url, request_body=body,
                                                  headers=headers, return_json=False)
    ...
    def allow_full_access_for_client(realm_name, client_name, get_service_user=False):
        try:
            ...
            raise AppException(f"Could not update access in realm client")
    ...
    def get_fastapi_idp_object(realm_name):
        ...
        try:
            return FastAPIKeycloak(
                ...
                realm=realm_name,
                callback_uri=(Utility.environment["idp"]["callback_uri"]).replace("REALM_NAME", realm_name)
            )
    ...
    def create_ipd_client(realm_name):
        ...
        try:
            ...
            body = body.replace("REDIRECT_URI",
                                (Utility.environment["idp"]["callback_uri"]).replace("REALM_NAME", realm_name))
