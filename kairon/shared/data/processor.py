...
class MongoProcessor:
    ...
    def get_logs_for_any_type(
        ...
        # DOC_TYPE_MAPPING = {
        #     LogTypeEnum.content: ContentValidationLogs,
        #     LogTypeEnum.importer: ValidationLogs,
        #     LogTypeEnum.history_deletion: ConversationsHistoryDeleteLogs,
        #     LogTypeEnum.multilingual: BotReplicationLogs,
        #     LogTypeEnum.catalog: CatalogSyncLogs,
        #     LogTypeEnum.custom_widget: CustomWidgetsRequestLog,
        #     LogTypeEnum.mail_channel: MailResponseLog,
        #     LogTypeEnum.callback: CallbackLog,
        #     LogTypeEnum.llm: LLMLogs,
        #     LogTypeEnum.actions: ActionServerLogs,
        #     LogTypeEnum.executor: ExecutorLogs,
        #     LogTypeEnum.agent_handoff: Metering,
        #     LogTypeEnum.audit: AuditLogData,
        # }
        #
        # logs = []
        # query = {}
        # if log_type in DOC_TYPE_MAPPING:
        #     doc_type = DOC_TYPE_MAPPING[log_type]
        #
        # if log_type == "llm":
        #     query = {"metadata__bot": bot}
        # elif log_type == "action_server_logs":
        #     query = {"bot": bot, "trigger_info.trigger_id": ""}
        # elif log_type == "callback_logs":
        #     query["bot"] = bot
        ...
        #         query["channel"] = channel
        #     if identifier := kwargs.get("identifier"):
        #         query["identifier"] = identifier
        # elif log_type == "executor":
        #     query["bot"] = bot
        #     if event_class := kwargs.get("event_class"):
        #         query["event_class"] = event_class
        #     if task_type := kwargs.get("task_type"):
        #         query["task_type"] = task_type
        # elif log_type == "agent_handoff":
        #     from_date = kwargs.get("from_date") or datetime.utcnow().replace(day=1, hour=0, minute=0, second=0,
        #                                                                      microsecond=0)
        #     to_date = kwargs.get("to_date") or datetime.utcnow().replace(
        ...
        #     })
        #
        #     logs_json = doc_type.objects(**query).order_by("-timestamp").skip(start_idx).limit(page_size).exclude(
        #         "id").to_json()
        #     count = MongoProcessor.get_row_count(doc_type, **query)
        #     return json.loads(logs_json), count
        #
        # elif log_type == "audit":
        #     from_date = kwargs.get("from_date") or datetime.utcnow().date()
        #     to_date = kwargs.get("to_date") or from_date + timedelta(days=1)
        ...
        #     logs_json = doc_type.objects(**query).order_by("-timestamp").skip(start_idx).limit(page_size).exclude(
        #         "id").to_json()
        #     count = MongoProcessor.get_row_count(doc_type, bot, **query)
        #     return json.loads(logs_json), count
        #
        # else:
        #     query["bot"] = bot
        #
        # sort_field = "-timestamp" if "timestamp" in doc_type._fields else "-start_timestamp"
        #
        # logs_cursor = doc_type.objects(**query).order_by(sort_field).skip(start_idx).limit(page_size)
        #
        ...
        #
        # count = doc_type.objects(**query).count()
        # return logs, count