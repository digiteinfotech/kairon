...
class ActionUtility:
    ...
    @staticmethod
    def execute_http_request(http_url: str, request_method: str, request_body=None, header=None):
        ...
        :param header: header for the HTTP request
        :return: JSON/string response
        """
        if not header:
            header = {}
        header.update({"User-Agent": "Mozilla/5.0 (compatible; Googlebot/2.1; +http://www.google.com/bot.html)"})
    ...
    @staticmethod
    def prepare_header(tracker: Tracker, auth_token=None, header_params: List[HttpActionRequestBody] = None):
        header = ActionUtility.prepare_request(tracker, header_params)
        if not ActionUtility.is_empty(auth_token):
            header.update({'Authorization': auth_token})
        return header
