...
class WhatsappBroadcast(MessageBroadcastFromConfig):
    ...
    async def send(self, recipients: List, **kwargs):
        if self.config["broadcast_type"] == MessageBroadcastType.static.value:
            ...
            await self.__send_using_pyscript()

    async def __send_using_pyscript(self):
        ...
        async def send_msg(template_id: Text, recipient, language_code: Text = "en", components: Dict = None, namespace: Text = None):
            response = await channel_client.send_template_message(template_id, recipient, language_code, components, namespace)
