...
class AccountProcessor:
    ...
    async def overwrite_password(token: str, password: str):
        ...
        if uuid_value is not None and Utility.is_exist(
                AuditLogData, raise_error=False, user=email, action=AuditlogActions.ACTIVITY.value, entity=UserActivityType.link_usage.value,
        ...
        if uuid_value is not None:
            AuditLogData.objects(user=email, action=AuditlogActions.ACTIVITY.value, entity=UserActivityType.link_usage.value,
                                 data__status="pending", data__uuid=uuid_value)\
