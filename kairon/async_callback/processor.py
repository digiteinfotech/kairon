...
def run_pyscript_async(script: str, predefined_objects: dict, callback: Any):
    ...
    async def execute_script_task(cb: Any, src_code: Text, pre_objs: Optional[dict] = None):
        try:
            data = run_pyscript(script=src_code, predefined_objects=pre_objs)
            await cb({'result': data})
        except AppException as e:
            await cb({'error': str(e)})
    ...
    try:
        if execution_mode == CallbackExecutionMode.ASYNC.value:
            async def callback_function(rsp: dict):
                copied_func = functools.partial(async_callback, rsp, entry, callback, callback_source, bot, entry.get("sender_id"), entry.get("channel"), request_data)
                await copied_func()

            await run_pyscript_async(script=callback.get("pyscript_code"), predefined_objects=predefined_objects, callback=callback_function)
        elif execution_mode == CallbackExecutionMode.SYNC.value:
            data = run_pyscript(script=callback.get("pyscript_code"), predefined_objects=predefined_objects)
